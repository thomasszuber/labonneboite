...
#TODO QUESTION : Check the behaviour of the pool of multiprocessing module
# https://docs.python.org/3/library/multiprocessing.html 
# Not sure how to use it, need a big load of data to make it work
# Need to pool.close or pool.join ? 
...
class GeocodeJob(Job):
    ...
    def run_missing_geocoding_jobs(self):
        def get_geocode_from_api(csv_path):
            ...
            if response.status_code == 200:
                try:
                    ...
                    pd_csv_api = pd.read_csv(io.StringIO(decoded_content))
                    ...
                    pd_csv_api = pd_csv_api[cols_of_interest]

                    for index, row in pd_csv_api.head().iterrows():
        ...
        nb_occur_csv = 80000
        list_csv_files = []
        for start in range(0, len(adresses_not_geolocated), nb_occur_csv):
            end = start + nb_occur_csv
            csv_path += str(start)+'-'+str(end)+'.csv'
            ...
            start = end
            list_csv_files.append(csv_path)
        ...
        for csv_path in list_csv_files:
            ...
            #TODO QUESTION : Do we need to remove the csv files from /tmp ?
